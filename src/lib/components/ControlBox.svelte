<script>
	...
	function handleMousemove(event) {
		tooltip = { x: event.offsetX, y: event.offsetY + 400, show: true };
	}
...
<div class="h-full w-full items-center controlBoxWrapper">
	<div class="font-bold text-white p-4 pb-0">선택 가능구역 : {completedParts} / {totalParts}</div>
	...
	<div class="flex justify-center items-center">
		<div class="flex gap-4 justify-between items-center w-5/6 buttons">
			<button
				...
				class="relative w-2/5 h-14 text-lg font-bold text-white rounded-full border-2 border-white duration-200 ease-in hover:text-gray-500 hover:bg-white button"
			>
				되돌리기
				<img
					src="icon/arrow.svg"
					alt="clockwise arrow icon"
					class="absolute top-4 right-3 w-5 md:invisible lg:visible icon"
				/>
			</button>
			<button
