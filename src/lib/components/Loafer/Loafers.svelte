<script>
	...
	import { getOldColorFromObject, getOldMaterialFromObject } from './undoFunctions.js';
			...
			if (intersects.length > 0) {
				...
				if (selectedObject.material) {
					if (
						...
						selectedObject.name !== 'Lion' &&
						selectedObject.name !== 'Coin'
					) {
						if ($selectedObjectName !== selectedObject.name) {
							selectedObjectName.set(selectedObject.name);
							selectedColor.set('');
